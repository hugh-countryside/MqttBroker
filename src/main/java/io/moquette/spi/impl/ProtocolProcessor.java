...
import static io.moquette.server.ConnectionDescriptor.ConnectionState.SUBSCRIPTIONS_REMOVED;
...
public class ProtocolProcessor {
    ...
    public void init(SubscriptionsDirectory subscriptions, IMessagesStore storageService,
    ...
    public void init(SubscriptionsDirectory subscriptions, IMessagesStore storageService,
        ...
        init(subscriptions, storageService, sessionsStore, authenticator, allowAnonymous,
    ...
    public void init(SubscriptionsDirectory subscriptions, IMessagesStore storageService,
        ...
        init(new ConnectionDescriptorStore(sessionsStore), subscriptions, storageService,
            sessionsStore, authenticator,
            allowAnonymous, allowZeroByteClientId, authorizator, interceptor, serverPort);
    }

    /**
     * @param subscriptions the subscription store where are stored all the existing clients
     * subscriptions.
     * @param storageService the persistent store to use for save/load of messages for QoS1 and QoS2
     * handling.
    ...
    void init(ConnectionDescriptorStore connectionDescriptors, SubscriptionsDirectory subscriptions,
        ...
        final PersistentQueueMessageSender messageSender = new PersistentQueueMessageSender(
            this.connectionDescriptors);
        this.messagesPublisher = new MessagesPublisher(connectionDescriptors, sessionsStore,
            messageSender,
            subscriptions);

        LOG.info("Initializing QoS publish handlers...");
        this.qos0PublishHandler = new Qos0PublishHandler(m_authorizator, m_messagesStore,
            m_interceptor,
            this.messagesPublisher);
        this.qos1PublishHandler = new Qos1PublishHandler(m_authorizator, m_messagesStore,
            m_interceptor,
            this.connectionDescriptors, this.messagesPublisher);
        this.qos2PublishHandler = new Qos2PublishHandler(m_authorizator, subscriptions,
            m_messagesStore, m_interceptor,
    ...
    public void processConnect(Channel channel, MqttConnectMessage msg) {
        ...
        if (clientId == null || clientId.length() == 0) {
            ...
            LOG.info("Client has connected with a server generated identifier. CId={}, username={}",
                clientId,
                payload.userName());
        }
        ...
        if (existing != null) {
            LOG.info(
                "The client ID is being used in an existing connection. It will be closed. CId={}",
                clientId);
            existing.abort();
            return;
        }
        ...
        if (Pattern.matches("stu_.*", clientId)) {
            ...
            MqttPublishMessage publishMessage = (MqttPublishMessage) MqttMessageFactory.newMessage(
                ...
                new MqttPublishVariableHeader("/sys/online", 0), Unpooled
                    .buffer().writeBytes(jsonObject.toJSONString().getBytes()));
            this.processPublish(channel, publishMessage);
        }
        LOG.info("The CONNECT message has been processed. CId={}, username={}", clientId,
            payload.userName());
    }
    ...
    private boolean login(Channel channel, MqttConnectMessage msg, final String clientId) {
        ...
        if (msg.variableHeader().hasUserName()) {
            ...
            if (!m_authenticator.checkValid(clientId, msg.payload().userName(), pwd)) {
                LOG.error(
                    "Authenticator has rejected the MQTT credentials CId={}, username={}, password={}",
    ...
    private boolean sendAck(ConnectionDescriptor descriptor, MqttConnectMessage msg,
        final String clientId) {
    ...
    private void initializeKeepAliveTimeout(Channel channel, MqttConnectMessage msg,
        final String clientId) {
        ...
        LOG.debug(
            "The connection has been configured CId={}, keepAlive={}, cleanSession={}, idleTime={}",
    ...
    private void storeWillMessage(MqttConnectMessage msg, final String clientId) {
        ...
        if (msg.variableHeader().isWillFlag()) {
            ...
            LOG.info(
                "Configuring MQTT last will and testament CId={}, willQos={}, willTopic={}, willRetain={}",
            ...
            WillMessage will = new WillMessage(msg.payload().willTopic(), bb,
    ...
    private ClientSession createOrLoadClientSession(ConnectionDescriptor descriptor,
        ...
        boolean isSessionAlreadyStored = clientSession != null;
        if (!isSessionAlreadyStored) {
            clientSession = m_sessionsStore
                .createNewSession(clientId, msg.variableHeader().isCleanSession());
        }
    ...
    public void processPubAck(Channel channel, MqttPubAckMessage msg) {
        ...
        InterceptAcknowledgedMessage wrapped = new InterceptAcknowledgedMessage(inflightMsg, topic,
            username, messageID);
    ...
    private static IMessagesStore.StoredMessage asStoredMessage(WillMessage will) {
        IMessagesStore.StoredMessage pub = new IMessagesStore.StoredMessage(
            will.getPayload().array(), will.getQos(),
            will.getTopic());
    ...
    /**
     * Intended usage is only for embedded versions of the broker, where the hosting application want
     * to use the broker to send a publish message. Inspired by {@link #processPublish} but with some
     * changes to avoid security check, and the handshake phases for Qos1 and Qos2. It also doesn't
     * notifyTopicPublished because using internally the owner should already know where it's
     * publishing.
     *
...
//        }
    ...
    public void processPubRec(Channel channel, MqttMessage msg) {
        ...
        MqttFixedHeader pubRelHeader = new MqttFixedHeader(MqttMessageType.PUBREL, false,
    ...
    public void processDisconnect(Channel channel) {
        ...
        channel.flush();
        final ConnectionDescriptor existingDescriptor;
        if (clientID != null && Pattern.matches("stu_.*", clientID)) {
            ...
            MqttPublishMessage publishMessage = (MqttPublishMessage) MqttMessageFactory.newMessage(
                new MqttFixedHeader(MqttMessageType.PUBLISH, true, MqttQoS.AT_MOST_ONCE, false, 0),
                new MqttPublishVariableHeader("/sys/offline", 0), Unpooled
                    .buffer().writeBytes(jsonObject.toJSONString().getBytes()));
            this.processPublish(channel, publishMessage);
        }
        if(clientID != null){
            existingDescriptor = this.connectionDescriptors.getConnection(clientID);
            ...
            if (existingDescriptor.doesNotUseChannel(channel)) {
                ...
                LOG.warn(
                    "Another client is using the connection descriptor. Closing connection. CId={}",
                    clientID);
    ...
    private boolean cleanWillMessageAndNotifyInterceptor(ConnectionDescriptor descriptor,
        String clientID) {
    ...
    public void processUnsubscribe(Channel channel, MqttUnsubscribeMessage msg) {
        ...
        for (String t : topics) {
            ...
            if (!validTopic) {
                ...
                channel.close();
                LOG.error("Topic filter is not valid. CId={}, topics={}, badTopicFilter={}",
        ...
        LOG.info("Sending UNSUBACK message. CId={}, topics={}, messageId={}", clientID, topics,
            messageID);
    ...
    public void processSubscribe(Channel channel, MqttSubscribeMessage msg) {
        ...
        SubscriptionState currentStatus = subscriptionInCourse
            .putIfAbsent(executionKey, SubscriptionState.VERIFIED);
        if (currentStatus != null) {
            LOG.warn(
                "Client sent another SUBSCRIBE message while this one was being processed CId={}, messageId={}",
                clientID, messageID);
            return;
        }
        String username = NettyUtils.userName(channel);
        ...
        if (!this.subscriptionInCourse
            .replace(executionKey, SubscriptionState.VERIFIED, SubscriptionState.STORED)) {
            LOG.warn(
                "Client sent another SUBSCRIBE message while the topic filters were being verified CId={}, "
                    +
                    "messageId={}", clientID, messageID);
            return;
        }

        LOG.info("Creating and storing subscriptions CId={}, messageId={}, topics={}", clientID,
            messageID, ackTopics);
        ...
        if (!success) {
            LOG.warn("Unable to perform the final subscription state update CId={}, messageId={}",
                clientID, messageID);
        }
    }

    private List<Subscription> doStoreSubscription(List<MqttTopicSubscription> ackTopics,
        String clientID) {
        ClientSession clientSession = m_sessionsStore.sessionForClient(clientID);
     ...
     */
    private List<MqttTopicSubscription> doVerify(String clientID, String username,
        ...
        for (MqttTopicSubscription req : msg.payload().topicSubscriptions()) {
            ...
            if (!m_authorizator.canRead(topic, username, clientSession.clientID)) {
                ...
                LOG.error(
                    "Client does not have read permissions on the topic CId={}, username={}, messageId={}, "
                        +
                ...
                if (topic.isValid()) {
                    LOG.info(
                        "Client will be subscribed to the topic CId={}, username={}, messageId={}, topic={}",
                        clientID, username, messageId, topic);
                    qos = req.qualityOfService();
                } else {
                    LOG.error(
                        "Topic filter is not valid CId={}, username={}, messageId={}, topic={}",
                        clientID,
     ...
     */
    private MqttSubAckMessage doAckMessageFromValidateFilters(
        List<MqttTopicSubscription> topicFilters, int messageId) {
    ...
    private void publishRetainedMessagesInSession(final Subscription newSubscription,
        String username) {
        ...
        ClientSession targetSession = m_sessionsStore
            .sessionForClient(newSubscription.getClientId());
    ...
    public void notifyChannelWritable(Channel channel) {
        ...
        while (channel.isWritable() && !emptyQueue) {
            ...
            if (msg == null) {
                ...
                MqttPublishMessage pubMsg = createPublishForQos(msg.getTopic(), msg.getQos(),
                    msg.getPayload(),
                    msg.isRetained(), 0);
